...
public class HuLiWeightWeekView extends LinearLayout {
	private Context context;
	public List<Integer> dayList = new ArrayList<Integer>();
	public List<Double> valueList = new ArrayList<Double>();
	private String uid;
	private HuLiWeightDB weightDB = null;

	public HuLiWeightWeekView(Context context) {
		super(context);
	...
	public HuLiWeightWeekView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.context = context;
		LayoutInflater.from(context).inflate(R.layout.huli_weight_view, this,
				true);
	...
	public void showCurve() {
		dayList.clear();
		valueList.clear();
		ArrayList<HuLiWeight> weightList = weightDB.getHuLiListByTime(new Date(),uid,
		...
		for (int i = 0; i < weightList.size(); i++) {
			try {
				dayList.add(getWeekOfDate(df.parse(weightList.get(i).date)));
